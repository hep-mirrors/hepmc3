...
FindParticles::FindParticles(const GenParticle &p, FilterParticle filter_type, FilterList filter_list) {
    ...
    switch(filter_type) {
        ...
        case FIND_MOTHERS:
            ...
            BOOST_FOREACH( const GenParticle &p_in, p.production_vertex().particles_in() ) {
        ...
        case FIND_DAUGHTERS:
            ...
            BOOST_FOREACH( const GenParticle &p_out, p.end_vertex().particles_out() ) {
        ...
        case FIND_PRODUCTION_SIBLINGS:
            ...
            BOOST_FOREACH( const GenParticle &p_in, p.end_vertex().particles_in() ) {
...
void FindParticles::recursive_check_ancestors(const GenVertex &v, FilterList &filter_list) {
    ...
    BOOST_FOREACH( const GenVertex &v_list, m_checked_vertices ) {
        if( v_list.barcode() == v.barcode() ) return;
    }
