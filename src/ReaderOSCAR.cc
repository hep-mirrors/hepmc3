...
bool ReaderOSCAR::read_event(GenEvent &evt) {
    ...
    while (!failed()) {
        ...
        if ( strncmp(buf, "HepMC", 5) == 0 ) {
            if ( strncmp(buf, "HepMC::Version", 14) != 0 && strncmp(buf, "HepMC::OSCARv3", 14) != 0 )
            {
                HEPMC3_WARNING("ReaderOSCAR: found unsupported expression in header. Will close the input.")
        ...
        case 'A':
            ...
            break;
        default:
            HEPMC3_WARNING("ReaderOSCAR: skipping unrecognised prefix: " << buf[0])
            is_parsing_successful = true;
            break;
        }
    ...
    if ((int)evt.particles().size() > vertices_and_particles.second) {
        HEPMC3_ERROR("ReaderOSCAR: too many particles were parsed")
        printf("%zu  vs  %i expected\n", evt.particles().size(), vertices_and_particles.second);
        is_parsing_successful = false;
    }
    if ((int)evt.particles().size() < vertices_and_particles.second) {
        HEPMC3_ERROR("ReaderOSCAR: too few  particles were parsed")
        printf("%zu  vs  %i expected\n", evt.particles().size(), vertices_and_particles.second);
        is_parsing_successful = false;
    }

    if ((int)evt.vertices().size()  > vertices_and_particles.first) {
        HEPMC3_ERROR("ReaderOSCAR: too many vertices were parsed")
        printf("%zu  vs  %i expected\n", evt.vertices().size(), vertices_and_particles.first);
        is_parsing_successful =  false;
    }

    if ((int)evt.vertices().size()  < vertices_and_particles.first) {
        HEPMC3_ERROR("ReaderOSCAR: too few vertices were parsed")
        printf("%zu  vs  %i expected\n", evt.vertices().size(), vertices_and_particles.first);
        is_parsing_successful =  false;
    }
    // Check if there were HEPMC3_ERRORs during parsing
    if ( !is_parsing_successful ) {
        HEPMC3_ERROR("ReaderOSCAR: event parsing failed. Returning empty event")
...
std::pair<int, int> ReaderOSCAR::parse_event_information(GenEvent &evt, const char *buf) {
    ...
    HEPMC3_DEBUG(10, "ReaderOSCAR: E: " << event_no << " (" <<ret.first << "V, " << ret.second << "P)")

    return ret;
}


bool ReaderOSCAR::parse_weight_values(GenEvent &evt, const char *buf) {
    ...
    if ( run_info() && run_info()->weight_names().size()
        ...
        throw std::logic_error("ReaderOSCAR::parse_weight_values: "
...
bool ReaderOSCAR::parse_units(GenEvent &evt, const char *buf) {
    const char *cursor = buf;
    ...
    HEPMC3_DEBUG(10, "ReaderOSCAR: U: " << Units::name(evt.momentum_unit()) << " " << Units::name(evt.length_unit()))

    return true;
}


bool ReaderOSCAR::parse_vertex_information(GenEvent &evt, const char *buf) {
    ...
    HEPMC3_DEBUG(10, "ReaderOSCAR: V: " << id << " with  "<< data->particles_in().size() << " particles)")
...
bool ReaderOSCAR::parse_particle_information(GenEvent &evt, const char *buf) {
    ...
    if ( atoi(cursor) != (int)evt.particles().size() + 1 ) {
        ...
        HEPMC3_ERROR("ReaderOSCAR: particle ID mismatch")
        return false;
    }
    ...
    else if ( mother_id < 0 )
    {
        ...
        if (!found)
        {
            ...
            //      WARNING("ReaderOSCAR: Unordered event, id of mother vertex  is out of range of known ids:   " <<mother_id<<" evt.vertices().size()="<<evt.vertices().size() )
    ...
    HEPMC3_DEBUG(10, "ReaderOSCAR: P: " << data->id() << " ( mother: " << mother_id << ", pid: " << data->pid() << ")")

    return true;
}


bool ReaderOSCAR::parse_attribute(GenEvent &evt, const char *buf) {
    const char     *cursor  = buf;
...
bool ReaderOSCAR::parse_run_attribute(const char *buf) {
    const char     *cursor  = buf;
    const char     *cursor2 = buf;
    char            name[512];
...
bool ReaderOSCAR::parse_weight_names(const char *buf) {
    const char     *cursor  = buf;
...
bool ReaderOSCAR::parse_tool(const char *buf) {
    const char     *cursor  = buf;
...
std::string ReaderOSCAR::unescape(const std::string& s) {
    std::string ret;
