...
{
if (gWriters.find(position)==gWriters.end()) { printf("Warning in %s: Writer at position %i does not exist\n",__FUNCTION__,position); return 1;}
if (!HepMC::hepevtptr) { printf("Error in %s: HEPEVT block does not exist\n",__FUNCTION__); 
	return 1; }//exit(1);}
...
//HepMC::HEPEVT_Wrapper::fix_daughters();
return 0;
}
...
int set_cross_section_(const int & position, const double& x,const double& xe)
{
