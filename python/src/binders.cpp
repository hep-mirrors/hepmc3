...
void custom_deduce_reader(pybind11::module&  M){
    M.def("ReaderGZ", [](const std::string & classname,const std::string & filename,const std::string & format) -> std::shared_ptr<class HepMC3::Reader>{ 
      ...
      if (classname == "ReaderAscii" ) return std::make_shared<HepMC3::ReaderAscii>(std::shared_ptr< std::istream >(new pystream::istream(zstdfile)));
      if (classname == "ReaderAsciiHepMC2" ) return std::make_shared<HepMC3::ReaderAsciiHepMC2>(std::shared_ptr< std::istream >(new pystream::istream(zstdfile)));
      if (classname == "ReaderHEPEVT" ) return std::make_shared<HepMC3::ReaderHEPEVT>(std::shared_ptr< std::istream >(new pystream::istream(zstdfile)));
      return nullptr;
