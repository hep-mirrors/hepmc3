...
int main()
{
    ReaderOSCAR1999 inputA("inputIO12.oscar1999A");
    if(inputA.failed()) return 1;
    WriterAscii       outputA("frominputIO12.hepmc");
    ...
    ReaderAscii inputB("frominputIO12.hepmc");
    ...
    while( !inputB.failed() )
    {
        ...
        if (!outputB)     outputB = std::make_shared<WriterOSCAR1997>("fromfrominputIO12.oscar1997A", evt.run_info());
