...
TauolaHepMC3Particle::TauolaHepMC3Particle( const HepMC3::GenParticle &particle) {
    m_particle = particle;
}
...
void TauolaHepMC3Particle::undecay() {
    ...
    if( m_particle.end_vertex() ) {
        ...
        HepMC3::GenVertex v = m_particle.end_vertex();

        // This will invalidate all downstream
        if(v) m_parent_event->delete_vertex(v);
    }
...
void TauolaHepMC3Particle::setMothers(vector<TauolaParticle*> mothers){
    ...
    if(mothers.size()>0) {
        ...
        if(!production_vertex) {
            production_vertex = HepMC3::GenVertex();
            parent_event()->add_vertex(production_vertex);
        }
...
void TauolaHepMC3Particle::decayEndgame() {
    ...
    HepMC3::FourVector new_position( previous_position + (tau_momentum/mass*lifetime) );
