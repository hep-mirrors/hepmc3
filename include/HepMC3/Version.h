...
#ifdef WIN32
#define DECLARE_HEPMC3_READER_FILE(classname)    extern "C" {  __declspec(dllexport) classname *  __stdcall new ## classname ## file (const std::string &filename ) { return new classname (filename);  } }
#define DECLARE_HEPMC3_READER_STREAM(classname)  extern "C" {  __declspec(dllexport) classname *  __stdcall new ## classname ## stream (std::istream & stream) { return new classname (stream);  } }
